...
func CheckAndCreate(dbPath string) error {
	...
	if _, err := os.Stat(dir); os.IsNotExist(err) {
		...
		if createErr != nil {
			return fmt.Errorf("ошибка создания директории: %w", createErr)
		}
	}
...
func initSchema(db *sql.DB, schemaPath string) error {
	...
	if err != nil {
		return fmt.Errorf("не удалось прочитать schema: %w", err)
	}
	_, err = db.Exec(string(schemaBytes))
	if err != nil {
		return fmt.Errorf("не удалось создать schema: %w", err)
	}
	return nil
}

// AddTask вставляет новую задачу в таблицу scheduler с указанной датой, заголовком, комментарием и правилом повторения.
// Возвращает результат ID новой задачи и любую обнаруженную ошибку.
func AddTask(date, title, comment, repeat string, db *sql.DB) (int64, error) {
...
LIMIT :limit`, sql.Named("pattern", pattern), sql.Named("limit", limit))
	...
	if err != nil {
		return []dto.Task{}, fmt.Errorf("Ошибка получения задач из базы данных: %w ", err)
	}
	defer func() {
	...
	for rows.Next() {
		...
		if err = rows.Scan(&task.ID, &task.Date, &task.Title, &task.Comment, &task.Repeat); err != nil {
			return []dto.Task{}, fmt.Errorf("ошибка сканирования задачи: %w", err)
		}
...
func GetTask(r *http.Request, database *sql.DB) (dto.Task, error) {
	...
	if err != nil {
		return dto.Task{}, fmt.Errorf("ошибка получения задачи из бд: %w", err)
	}
	return task, nil
}
