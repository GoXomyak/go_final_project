...
// TaskValidator выполняет валидацию запроса задачи.
//
// Выполняет:
// - проверку корректности формата полей
// - проверку логических ограничений
// - автоматическую корректировку полей (где это возможно)
//
// Принимает:
// - TaskRequest для валидации
//
// Возвращает:
// - TaskRequest: проверенный и скорректированный запрос
// - error: ошибка при неуспешной валидации
//
// Примечание:
// Валидатор может модифицировать исходный запрос для приведения
// его к корректному формату, если это возможно без нарушения
// бизнес-логики
